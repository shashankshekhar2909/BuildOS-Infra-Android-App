    ...
    fun resetSandboxData() {
        ...
        val initialNodes = listOf(
            Node(
                ...
                status = "online",
                lastPing = "2s ago",
                agentToken = "bo_node_agent_tok_de218acb9c02",
                installCommand = "curl -fsSL https://os.buildwithshashank.com/install.sh | bash -s -- --token bo_node_agent_tok_de218acb9c02"
            ),
            Node(
                id = "node-abc-2",
                name = "standalone_docker_aws",
                type = "DOCKER",
                provider = "AWS",
                ip = "54.210.12.99",
                region = "us-east-1",
                status = "online",
                lastPing = "14s ago",
                agentToken = "bo_node_agent_tok_ff219ac023",
                installCommand = "curl -fsSL https://os.buildwithshashank.com/install.sh | bash -s -- --token bo_node_agent_tok_ff219ac023"
            ),
            Node(
                id = "node-abc-3",
                name = "core_auth_manager",
                type = "STANDALONE",
                ...
                status = "warning",
                lastPing = "2m ago",
                agentToken = "bo_node_agent_tok_ab901f4c7d03",
                installCommand = "curl -fsSL https://os.buildwithshashank.com/install.sh | bash -s -- --token bo_node_agent_tok_ab901f4c7d03"
            ),
            Node(
                id = "node-abc-4",
                name = "backup_node04",
                ...
                status = "offline",
                lastPing = "5h ago",
                agentToken = "bo_node_agent_tok_e921dcd88e99",
                installCommand = "curl -fsSL https://os.buildwithshashank.com/install.sh | bash -s -- --token bo_node_agent_tok_e921dcd88e99"
            )
        )
        ...
        _containers.value = listOf(
            Container(
                id = "cont-1",
                name = "buildos-main-web",
                nodeId = "node-abc-2",
                nodeName = "standalone_docker_aws",
                image = "node:20-alpine",
                state = "running",
                ports = "80:3000, 443:3000",
                domain = "os.buildwithshashank.com",
                autoHeal = true
            ),
            Container(
                id = "cont-2",
                name = "buildos-gateway-api",
                nodeId = "node-abc-2",
                nodeName = "standalone_docker_aws",
                image = "buildwithshashank/buildos-api:latest",
                state = "running",
                ports = "8000:8000",
                domain = "api.os.buildwithshashank.com",
                autoHeal = true
            ),
            Container(
                id = "cont-3",
                name = "cloudflare-daemon-tunnel",
                nodeId = "node-abc-2",
                nodeName = "standalone_docker_aws",
                image = "cloudflare/cloudflared:latest",
                state = "running",
                ports = "No exposed ports",
                domain = "tunnel.os.buildwithshashank.com",
                autoHeal = true
            ),
            Container(
                id = "cont-4",
                ...
                nodeId = "node-abc-3",
                nodeName = "core_auth_manager",
                image = "postgres:15-alpine",
                state = "running",
                ports = "5432:5432",
                domain = "db-replica.internal",
                autoHeal = false
            ),
            Container(
                id = "cont-5",
        ...
        _zones.value = listOf(
            CloudflareZone(id = "zone-1", name = "buildwithshashank.com", status = "active", zoneHealth = "Active / Accessible"),
            CloudflareZone(id = "zone-2", name = "shashank.dev", status = "active", zoneHealth = "Active / Checked"),
            CloudflareZone(id = "zone-3", name = "infra-deploy.sh", status = "pending", zoneHealth = "Nameserver Setup Pending")
        )

        _dnsRecords.value = mapOf(
            "zone-1" to listOf(
                DnsRecord(id = "rec-1", zoneId = "zone-1", name = "os.buildwithshashank.com", type = "A", content = "54.210.12.99", proxied = true),
                DnsRecord(id = "rec-2", zoneId = "zone-1", name = "api.os.buildwithshashank.com", type = "CNAME", content = "standalone.aws.domain", proxied = true),
                DnsRecord(id = "rec-3", zoneId = "zone-1", name = "ssh.os.buildwithshashank.com", type = "A", content = "142.250.190.46", proxied = false)
            ),
            "zone-2" to listOf(
                DnsRecord(id = "rec-4", zoneId = "zone-2", name = "api.shashank.dev", type = "A", content = "188.166.44.22", proxied = true),
                DnsRecord(id = "rec-5", zoneId = "zone-2", name = "blog.shashank.dev", type = "CNAME", content = "gh-pages.github.com", proxied = false)
            ),
            "zone-3" to emptyList()
        )

        _proxmoxGuests.value = mapOf(
            "node-abc-1" to listOf(
                ProxmoxGuest(vmid = 100, name = "web-stage-prod", status = "running", type = "qemu", cpu = 4.2, mem = 2147483648L, maxmem = 4294967296L),
                ProxmoxGuest(vmid = 101, name = "lxc-infra-dns", status = "running", type = "lxc", cpu = 0.8, mem = 536870912L, maxmem = 1073741824L),
                ProxmoxGuest(vmid = 102, name = "pve-runner-01", status = "stopped", type = "qemu", cpu = 0.0, mem = 0L, maxmem = 8589934592L)
            )
        )

        _logs.value = listOf(
            SystemLog("log-1", getFormattedTime(0), "INFO", "auth-manager", "Admin login successful for admin from client user-agent"),
            SystemLog("log-2", getFormattedTime(1), "INFO", "dns-syncer", "Cloudflare synchronization event completed successfully. 5 records validated."),
            SystemLog("log-3", getFormattedTime(2), "WARN", "node-agent", "Node backup_node04 is unresponsive. Ping timed out after 10000ms."),
            SystemLog("log-4", getFormattedTime(3), "INFO", "container-monitor", "Auto-heal engine successfully started backup health checker."),
            SystemLog("log-5", getFormattedTime(4), "ERROR", "standalone_docker_aws", "Container redis-cache-tier is down. Auto-heal is disabled, skipping recovery.")
        )
    }
